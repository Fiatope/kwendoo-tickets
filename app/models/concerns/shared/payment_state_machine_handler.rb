    ...
    state_machine :state, initial: :pending do
      ...
      after_transition do |resource, transition|
        ...
        if resource.is_a? Contribution
          ...
          resource.notify_observers :update_resource_project_total
          if transition.to == "confirmed"
            resource.generate_tickets
            Rails.cache.delete('user_tickets') if Rails.cache.exist?('user_tickets')
            Rails.cache.delete('user_no_free_tickets') if Rails.cache.exist?('user_no_free_tickets')
            # resource.notify_owner(:booking_confirmed)
          end
        end
      end
    end
