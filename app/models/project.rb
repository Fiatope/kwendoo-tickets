class Project < ActiveRecord::Base
  ...
  delegate :display_status,
           ...
           :display_somme_nette,
           :display_total_payment_service_fee,
           :display_goal,
           :remaining_days,
           :progress_bar,
           :successful_flag,
  ...
  has_many :ticket_categories_orders, through: :contributions
  ...
  accepts_nested_attributes_for :reward_categories
  accepts_nested_attributes_for :project_documents

  delegate :somme_nette, :progress, :total_contributions, :net_amount, :platform_fee,
  ...
  scope :featured, -> { where(featured: true).where.not(state: ["draft", "deleted", "failed"]) }
  ...
  scope :order_for_search, ->{ reorder(Arel.sql("
    CASE projects.state
    WHEN 'online' THEN 1
  ...
  scope :with_contributions_canceled_today, -> {
    joins(:contributions).merge(Contribution.canceled_today).uniq
  }
  ...
  validates_presence_of :name, :user, :category, :about, :headline, :goal, :permalink, :location, :start_date
  validates_inclusion_of :is_prebooked, in: [true, false]
  ...
  before_validation do
    if self.start_date.present?
      self.online_days = self.start_date - Date.today
    end
  end

  # def currency
  #   Currency::SYMBOLS[self.address_state]
  # end
  ...
  def pledged
    contributions.where(state: "confirmed").map(&:value).sum
  end


  def has_at_least_one_cfa_contribution?
    contributions.with_state(:confirmed).where(payment_method: "Orange Money").any?
  end
  ...
  def expires_at
    ...
    # online_date && (online_date + online_days.days).end_of_month
    #online_date && (online_date + online_days.days).new
  end
  ...
  def expired?
    rewards.all?(&:sold_out?) ||
      expires_at && expires_at < Time.zone.now
  end

  #before expired? was expired_at && expires_at < Time.zone.now
  ...
  def is_finalized?
    !(state == 'request_funds') && !(state == 'draft') && !(state == 'fraud_suspiscion') && !(state == 'fraud_suspiscion_or_cancelled')
  end

  def funds_can_be_withdrawn?
    is_finalized? && user.has_kycs_required_to_withdraw_funds? && user.bank_information.try(:iban)
  end
